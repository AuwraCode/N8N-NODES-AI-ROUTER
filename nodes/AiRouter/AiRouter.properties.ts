...
export const AI_ROUTER_PROPERTIES: INodeProperties[] = [
...

  {
    ...
    type: 'boolean',
    default: true,
    description: 'Whether to automatically retry with the next-best model when the primary model returns a 429, 5xx, or network error (up to 3 attempts total)',
  },
  {
    ...
    type: 'boolean',
    default: false,
    description: 'Whether to skip the API call and return only the routing decision (selected model, score, detected task). No tokens are spent in dry-run mode.',
  },
  {
...

  {
    ...
    type: 'boolean',
    default: false,
    description: 'Whether to add modelUsed, providerUsed, attemptsTaken, and token counts to the output JSON',
  },
  {
    ...
    type: 'boolean',
    default: false,
    description: 'Whether to add detectedTask and detectedTaskConfidence to the output (useful for debugging task classification)',
  },
  {
    ...
    type: 'boolean',
    default: false,
    description: 'Whether to add scoreBreakdown to the output — the top-3 ranked candidate models with their final scores and per-criterion sub-scores (taskFit, cost, latency, contextSize)',
  },
  {
    ...
    type: 'boolean',
    default: false,
    description: 'Whether to add estimatedCostUSD to the output, calculated from token counts and registry pricing (not available in dry-run mode)',
